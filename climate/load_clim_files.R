...
rain = read_clim("long_drought.rain")
tmin = read_clim("long_drought.tmin")
tmax = read_clim("long_drought.tmax")

annrain <- rain %>% group_by(wy) %>% summarise(totrain = sum(rain)) 
summary(annrain)
annrain%>% 
  ggplot(aes(wy, y=totrain)) + 
  geom_hline(yintercept=mean(annrain$totrain), col="red") + 
  geom_col()

tmin %>% group_by(month, year) %>% 
  summarise(tmin = mean(tmin)) %>% 
  ggplot(aes(x=year, y=tmin, col=month)) + 
  geom_point() + scale_colour_viridis_c() + 
  geom_smooth(method='lm')

inner_join(tmin, tmax) %>% 
  group_by(month, year) %>% 
  summarise(tmin = mean(tmin), tmax=mean(tmax)) %>% 
  ggplot() + 
  geom_point(aes(x=year, y=tmin), col="blue") + 
  geom_point(aes(x=year, y=tmax), col="red") +
  geom_smooth(method='lm', aes(x=year, y=tmin), col="blue") +
  geom_smooth(method='lm', aes(x=year, y=tmax), col="red") 
