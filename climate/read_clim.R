read_clim <- function (clim_in, dates_out = FALSE) 
{
    ...
    opts = c(".rain", ".tmin", ".tmax", ".tavg", ".dayl", ".daytime_rain_duration", 
        ".LAI_scalar", ".Ldown", ".Kdown_direct", ".Kdown_diffuse", 
        ".ndep_NO3", ".ndep_NH4", ".PAR_direct", ".PAR_diffuse", 
        ".relative_humidity", ".tday", ".tnightmax", ".tsoil", 
    ...
    if (length(files_in) == 0) {
        warning("Could not find any clim files matching path and prefix: ", 
            clim_in, ". Returning 'NULL'.")
        return(NULL)
    }
    read_in = lapply(files_in, readLines)
    starts_in = sapply(read_in, "[[", 1)
    lengths_in = sapply(read_in, length)
    trimstart = sapply(strsplit(starts_in, "\\s+"), function(x) paste(x[1], 
        x[2], x[3]))
    start_dates = as.POSIXct(trimstart, format = "%Y %m %d")
    datefun = function(x, y) {
        seq.POSIXt(from = x, by = "DSTday", length.out = y - 
            1)
    }
    date_seqs = mapply(datefun, start_dates, lengths_in, SIMPLIFY = FALSE)
    dataonly = mapply(function(x) as.numeric(x[2:length(x)]), 
        read_in, SIMPLIFY = FALSE)
    premerge = mapply("data.frame", date = date_seqs, dataonly, 
        stringsAsFactors = FALSE, SIMPLIFY = FALSE)
    clim = Reduce(function(x, y) merge(x = x, y = y, by = "date", 
        all = TRUE), premerge)
    names(clim)[2:ncol(clim)] = gsub("\\.", "", unlist(str_extract_all(files_in, 
        str_c(opts, collapse = "|"))))
    if (dates_out) {
        start_end = as.Date(c(min(clim$date, na.rm = T), max(clim$date, 
            na.rm = T)), format = "%m/%d/%y")
    ...
    clim$wy = data.table::fifelse(clim$month >= 10, clim$year + 
        1, clim$year)
    clim$yd = lubridate::yday(clim$date)
    wy_date = c(clim$date[93:length(clim$date)], seq.POSIXt(from = clim$date[length(clim$date)], 
        by = "DSTday", length.out = 93)[2:93])
